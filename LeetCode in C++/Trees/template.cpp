...
int minheight(vector<int> &inorder, map<int, int> &index_map, int left, int right)
{
	if (left>right)
	{
		return 0;
	...
	return 1+max(minheight(inorder, index_map, left, mid-1),minheight(inorder, index_map, mid+1, right));
}
int main()
{
